...
```
To edit a sale, send a PUT request to the listed URl with the ID corresponding to the sale you wish to edit. Change the format to JSON. The JSON should be formatted just the same as when creating a sale. Simply replace the old value with the new one.
